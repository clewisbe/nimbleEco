...
make.prior <- function(term, prior,...){
  ...
  if (prior=="Normal"){
    ...
    substitute(TERM ~ dunif(0, MAX), list(TERM=term, MAX = param$ub))
  }

  else if (prior=="t"){
    substitute(TERM ~ dt(0, 1, D), list(TERM=term, D = param$d))
  }
...
make.glm <- function(RHS, factors = "None", cl, mixture, link, N = NULL, Site = NULL, level = NULL){
...
lmPredRandom2PRIOR<- function(termparen, RHSname, factors, index.name, prior, dropbase, int.check){
  ...
  if(term[1] == 1){
    ...
    #Priors
    prior.sd <-  make.prior(make.sigma.name(param.name),"Uniform", ub = 1000)
