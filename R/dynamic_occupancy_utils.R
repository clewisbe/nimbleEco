...
#Takes 4 Data Objects and Returns Tidy Data Frame for Dynamic Occupancy Models
# Data should be entered as follows: y is array (site, survey, season), site is matrix or df,
...
tidy.dynam.occ <- function(y = NULL, site = NULL, season.site = NULL, season.site.survey = NULL){
  ...
  if(is.null(y)) {stop('Must Provide 0/1 Observation Data, y')}
  ...
  if(!is.null(season.site)){
    ...
    for (i in seq_along(season.site)){
      ...
      if(nseason != dim(mf)[2]){stop('Number of Seasons do Not Match Between Y and Covariates')}
  ...
  if(!is.null(season.site.survey)){
    ...
    for (i in seq_along(season.site.survey)){
      if(is.array(season.site.survey[[i]]) == FALSE) {stop('Site Level Covariates Need to be Provided as An Array')}
      mf <- melt(season.site.survey[[i]], varnames = c("Site", "Survey", "Season"), value.name = obs.vars[i])
      if(dim(y.tidy)[1] != dim(mf)[1]) {stop('Number of Observations do Not Match Between Y and Covariates')}
      covariates[[i + 1]] = mf
    }
  ...
  else{season.site.survey.tidy = NULL}
...
latent.mean.loop <- function(nsite, nseason){
  fin = nseason - 1
  l1 <- substitute(muZ[(T*(s-1) + 1):(T*s)] <- z[(T*(s - 1) + 1):(T*s)]*phi[(T*(s-1) + 1):(T*s)] + (1 - z[(T*(s-1) + 1):(T*s)])*gamma[(T*(s-1) + 1):(T*s)],
                   list(T = nsite))
...
make.obs.loop <- function(nsite, nseason, nsurvey){
  ...
  return(embedLinesInForLoop(list(l1, l2), quote(i), start = 1, finish = nsite*nseason*nsurvey)) #Add {} gap in code generation for redability
}
