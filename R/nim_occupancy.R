...
#'
#'
#' @param siteformula  A linear model formula for the latent occupancy state: \eqn{logit(\psi_{i})}.  Formula must start with a tilde (~).  Add 1 for an intercept.
...
#' @param y Detection and non detection data.  Must be provided as either a matrix or dataframe. Rows are sites and columns are replicates.  1 is detection and 0 is non detection.
